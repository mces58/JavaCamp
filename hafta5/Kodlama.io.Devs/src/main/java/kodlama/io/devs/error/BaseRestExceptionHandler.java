...
public class BaseRestExceptionHandler {
	...
	// girilen tarihte format hatasi olursa 400 hatasi firlat
	@ExceptionHandler(value = InvalidDataAccessResourceUsageException.class)
	public ResponseEntity<?> handleException(InvalidDataAccessResourceUsageException exception) {
		BaseErrorResponse error = new BaseErrorResponse();

		// degerleri set et
		error.setStatus(HttpStatus.BAD_REQUEST.value());
		error.setMessage("Tarihte format hatası: " + exception.getRootCause());
		error.setTimeStamp(LocalDateTime.now());
